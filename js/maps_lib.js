...
var MapsLib = {
  ...
  //Using the v1 Fusion Tables API. See https://developers.google.com/fusiontables/docs/v2/migration_guide for more info

  //MODIFY the encrypted Table IDs of your Fusion Tables (found under File => About)
  //NOTE: numeric IDs will be depricated soon
  fusionTableId:      "1oIU-tjSG85f4OVNoe7y7pTuPjZM1_AGHJ3YjSngw", //Moormeldingen data layer
  
  polygon1TableID:    "17TcDCEIBtdXf2DQhyndkPHIGMX-WJ3GWLXLN61BR", // bbk amsterdam
  polygon2TableID:    "1zPhBV7pZcF99q_asxroP0pDckiAIYFQIEwjXpane", //Bodemkwaliteitskaart
  ...
  map_centroid:       new google.maps.LatLng(52.373116, 4.858884), //center that your map defaults to
  ...
  //-----custom filters for point data layer
    ...
    var type_column = "'Type_Werk2'";  // -- note use of single & double quotes for two-word column header
    var tempWhereClause = [];
    if ( $("#cbType1").is(':checked')) tempWhereClause.push("BUS");
    if ( $("#cbType2").is(':checked')) tempWhereClause.push("melding");
  ...
  getList: function(whereClause) {
    ...
    var selectColumns = "Status, netbeheerder, Address";
  ...
  displayList: function(json) {
    ...
    if (rows == null) {
      ...
      // rows[row][0] = Status
      // rows[row][1] = Netbeheerder
      // rows[row][2] = Type werk2
      // rows[row][3] = Type werk
      // rows[row][4] = Beheerder Openbare Ruimte
      // rows[row][5] = Aannemer netbeheerder
      // rows[row][6] = Address
      // rows[row][7] = no use
      // rows[row][8] = no use
      ...
      for (var row in rows) {
        ...
        var school = "<a href='" + rows[row][1] + "'>" + rows[row][2] + "</a>";
        ...
        list_table += "\
          <tr>\
            ...
            <td>" + address + "</td>\
            <td>" + rows[row][6] + "</td>\
            <td><span data-value='" + rows[row][3] + "'><img src='" + rows[row][1] + "' /></span></td>\
          </tr>";
      }
